...
def short_term(num_slices, num_UEs, num_RUs, num_RBs, rb_bandwidth, P_i, gain, R_min, epsilon, arr_pi_sk, arr_phi_i_sk, logger=None):
    try:
        ...
        if problem.status == cp.OPTIMAL:
            return extract_values(short_pi_sk, int), extract_values(short_z_ib_sk, int), extract_values(short_p_ib_sk, float), extract_values(short_mu_ib_sk, float), short_total_R_sk.value
                
        return short_pi_sk, short_z_ib_sk, short_p_ib_sk, short_mu_ib_sk, short_total_R_sk.value 
...
def long_term(num_slices, num_UEs, num_RUs, num_DUs, num_CUs, num_RBs, P_i, rb_bandwidth, D_j, D_m, R_min, gain, A_j, A_m, l_ru_du, l_du_cu, epsilon, gamma, slice_mapping, logger=None):
    try:
        ...
        if problem.status == cp.OPTIMAL:
            return extract_values(pi_sk, int), extract_values(z_ib_sk, int), extract_values(p_ib_sk, float), \
                    extract_values(mu_ib_sk, float), extract_values(phi_i_sk, int), extract_values(phi_j_sk, int), \
                        extract_values(phi_m_sk, int), total_R_sk.value
